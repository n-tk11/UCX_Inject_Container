...
#	echo "%3 wlibfabric"
#	srun --mpi=pmi2 -n $NNODE singularity exec --bind $LIBF_BIND $WORKDIR/osu_container/osu_mpich_wlibf.sif /opt/$OSU_ALLREDUCE
	echo "%4 no ucx"
	srun --mpi=pmi2 -n $NNODE singularity exec --bind $LIBIB_BIND $WORKDIR/osu_container/osu_noucx_mpich.sif /opt/$OSU_ALLREDUCE
	echo "%5 mpi repl"
	srun --mpi=pmi2 -n $NNODE singularity exec --bind $MPI_REPL $WORKDIR/osu_container/osu_noucx_mpich.sif /opt/$OSU_ALLREDUCE
