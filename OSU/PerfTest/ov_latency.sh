#!/bin/bash
#SBATCH --job-name overhead_osu_latency_nin
...
echo "%1 OV1_latency"
srun --mpi=pmi2 -n 2 $WORKDIR/osu_mpich/$OSU_LATENCY
echo "%2 OV2_latency"
srun --mpi=pmi2 -n 2 singularity exec --bind $BIND_ARG2 $WORKDIR/osu_container/osu_noib_ubuntu22.sif /opt/libexec/osu-micro-benchmarks/mpi/pt2pt/osu_latency

echo "%3 OV3_latency"
srun --mpi=pmi2 -n 2 singularity exec --bind $LIBIB_BIND $WORKDIR/osu_container/osu_noib_ubuntu22.sif /opt/libexec/osu-micro-benchmarks/mpi/pt2pt/osu_latency

echo "%4 OV4_latency"
srun --mpi=pmi2 -n 2 singularity exec $WORKDIR/osu_container/osu_noucx_mpich.sif /opt/libexec/osu-micro-benchmarks/mpi/pt2pt/osu_latency

echo "%5 OV5_latency"
srun --mpi=pmi2 -n 2 singularity exec --bind $MPI_REPL $WORKDIR/osu_container/osu_noucx_mpich.sif /opt/libexec/osu-micro-benchmarks/mpi/pt2pt/osu_latency
